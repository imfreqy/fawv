...
// Decode/normalize a key. Supports s3://, https://..., and raw/percent-encoded keys.
function coerceKey(any) {
  if (!any) return null;
  const s = String(any);
  if (s.startsWith("s3://")) {
  ...
  try {
    // decode if percent-encoded, else returns original
...
// Parse bucket/key from body. Prefer manifestRef (s3:// or https), else use manifestKey|key|ref and S3_BUCKET/VAULT_BUCKET.
function resolveLocation({ key, manifestKey, manifestRef, ref, bucket: bodyBucket }) {
  if (manifestRef) {
  ...
  const finalKey = coerceKey(key || manifestKey || ref);
  const bucket = bodyBucket || process.env.VAULT_BUCKET || process.env.S3_BUCKET;
  if (!finalKey) throw new Error("key|manifestKey|ref or manifestRef is required");
...
// Deep merge utility for manifest.extra
...
// POST /api/manifest/force-extra  { manifestKey|manifestRef|key|ref, [bucket], extra: { manifestText, ... } }
router.post("/manifest/force-extra", async (req, res) => {
  console.log("[force-extra] entry", { bodyKeys: Object.keys(req.body||{}), sample: { manifestKey: req.body?.manifestKey, key: req.body?.key, ref: req.body?.ref, manifestRef: req.body?.manifestRef } });
  try {
    const { bucket, key, hintRegion } = (() => {
      const r = resolveLocation(req.body || {});
      return { bucket: r.bucket, key: r.key, hintRegion: r.hintRegion };
    ...
    const { extra, manifestText } = req.body || {};
    const s3 = await getS3ForBucket(bucket, hintRegion);

    // Read existing (ok if missing -> start fresh object)
    let obj = {};
    try {
    ...
    // Patch: deep-merge provided `extra`, and if `manifestText` is a string, set/override it
    obj.extra = deepMerge(obj.extra, typeof extra === "object" ? extra : {});
    ...
    if (process.env.S3_SSE) putParams.ServerSideEncryption = process.env.S3_SSE;

    await s3.send(new PutObjectCommand(putParams));

    return res.json({ ok: true, bucket, key, updatedAt: obj.updatedAt });
  } catch (e) {
