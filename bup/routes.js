...
// ------------------------------ Vault presign + list helpers ------------------------------
async function listUnderPrefix(prefix) {
  const Bucket = S3_BUCKET;
  if (!Bucket) throw new Error("missing_S3_BUCKET");
  const s3c = await getS3ForBucket(Bucket, null);
  let ContinuationToken = undefined;
  const out = [];
  do {
    const res = await s3c.send(new ListObjectsV2Command({ Bucket, Prefix: prefix, ContinuationToken }));
    (res.Contents || []).forEach((obj) => {
      out.push({ key: obj.Key, size: obj.Size, lastModified: obj.LastModified?.toISOString?.() });
    });
...
async function presignGetKey(key, expiresIn = 3600) {
  const Bucket = S3_BUCKET;
  if (!Bucket) throw new Error("missing_S3_BUCKET");
  const s3c = await getS3ForBucket(Bucket, null);
  const cmd = new GetObjectCommand({ Bucket, Key: key });
  const url = await getSignedUrl(s3c, cmd, { expiresIn });
  return url;
}

// ------------------------------ Vault actions routes ------------------------------
router.post("/presign-get", async (req, res) => {
  try {
    const { key, expiresIn } = req.body || {};
    if (!key) return res.status(400).json({ ok: false, error: "bad_request", message: "key required" });
    const url = await presignGetKey(key, expiresIn || 3600);
    res.json({ ok: true, url });
  } catch (err) {
    console.error("[presign-get] error", err);
    res.status(500).json({ ok: false, error: "presign_get_failed", message: err?.message || "UnknownError" });
  }
});
...
router.post("/vault/list", async (req, res) => {
  try {
    ...
    if (!prefix) return res.status(400).json({ ok: false, error: "bad_request", message: "prefix required" });
    const objects = await listUnderPrefix(prefix);
    res.json({ ok: true, objects });
  } catch (err) {
    console.error("[vault/list] error", err);
    res.status(500).json({ ok: false, error: "vault_list_failed", message: err?.message || "UnknownError" });
  }
});
